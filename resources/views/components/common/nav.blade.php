<nav class="bg-violet-50 w-full z-20 top-0 left-0 fixed mb-3">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
      <a href="{{route('root')}}" class="flex items-center">
        <img src="{{ asset('storage/images/lists-logo.png') }}" class="h-8 mr-3" alt="Todo-Lists Logo">
      ...
      <div class="flex md:order-2">
        ...
        <form action="{{ route('login.destroy')}}" method="post">
          @method('DELETE')
          @csrf
        <button type="submit" class="text-white bg-violet-700 font-bold hover:bg-violet-800 focus:ring-4 focus:outline-none focus:ring-violet-300 bb? rounded-lg text-sm px-4 py-2 text-center mr-3 md:mr-0">Logout</button>
        </form>
        @endauth
      ...
      <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-sticky">
        <ul class="flex flex-col p-4 md:p-0 mt-4 font-medium rounded-lg bg-violet-50 md:flex-row md:space-x-8 md:mt-0">
          ...
          <li>
            <a href="#About" class="block py-2 pl-3 pr-4 text-violet-900 rounded md:bg-transparent md:hover:text-violet-700 md:p-0">About</a>
          </li>
          <li>
            <a href="#Services" class="block py-2 pl-3 pr-4 text-violet-900 rounded md:bg-transparent md:hover:text-violet-700 md:p-0">Services</a>
          </li>
          <li>
